...
movies_blueprint = Blueprint( 'movies' , url_prefix='/movies' )

@movies_blueprint.route( '/' )
def commands_root( request ):
	return response.text( "you are at the localstorage:11301/api/movies url\n" )

@movies_blueprint.route( "/pause" , methods=[ "GET" ] )
def pause( request ):
...
@movies_blueprint.route( "/resume" , methods=[ "GET" ] )
def resume( request ):
	result = { "message": "failed" }
	try:
		vlc_config = get_vlc_config_from_redis()
...
@movies_blueprint.route( "/play" , methods=[ "GET" ] )
def play( request ):
	result = { "message": "failed" }
	try:
		vlc_config = get_vlc_config_from_redis()
...
@movies_blueprint.route( "/stop" , methods=[ "GET" ] )
def stop( request ):
	result = { "message": "failed" }
	try:
		vlc_config = get_vlc_config_from_redis()
...
@movies_blueprint.route( "/previous" , methods=[ "GET" ] )
...
@movies_blueprint.route( "/next" , methods=[ "GET" ] )
def next( request ):
	result = { "message": "failed" }
	try:
		vlc_config = get_vlc_config_from_redis()
...
@movies_blueprint.route( "/status" , methods=[ "GET" ] )
def status( request ):
	result = { "message": "failed" }
	try:
		vlc_config = get_vlc_config_from_redis()
